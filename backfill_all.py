...
def backfill_ticker(ticker, name, market):
    ...
    print(f"  백필 대상: {len(to_fill)}일 ({start_d} ~ {end_d}), market={market}")
    ...
    if df is not None:
        ...
        print(f"  기간별 조회 성공: {len(rate_map)}일")
    else:
        # ── 방법 B: 날짜별 루프 (by_ticker) ──────────────────────
        print(f"  기간별 실패 → 날짜별 루프...")
        ...
        for i, d in enumerate(all_dates):
            try:
                ...
                if i % 20 == 0:
                    print(f"    {i+1}/{len(all_dates)}일 처리중... ({len(rate_map)}일 수집)")
            except Exception as e:
                time.sleep(1)
        print(f"  날짜별 루프 완료: {len(rate_map)}일")
