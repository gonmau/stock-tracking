...
def make_chart(df: pd.DataFrame, days: int) -> go.Figure:
    ...
    for level, color, opacity in [
        (T["danger"],  "rgba(255,80,80,0.06)",   True),
        (T["warning"], "rgba(255,210,60,0.05)",  True),
    ]:
        fig.add_hrect(
            y0=level, y1=level + 60,
    ...
    fig.add_trace(go.Bar(
        ...
        name="당일공매도", marker_color=bar_colors,
        marker_line_width=0,
    ), row=2, col=1)
    ...
    fig.add_trace(go.Bar(
        ...
        name="거래량", marker_color="#44444488",
        marker_line_width=0,
    ), row=3, col=1)
    ...
    fig.update_layout(
        ...
        yaxis2=dict(title="잔고(만주)", overlaying="y", side="right",
                    showgrid=False, ticksuffix="만"),
        yaxis3=dict(ticksuffix="천"),
        xaxis3=dict(tickformat="%m/%d"),
    )
    return fig
...
def main():
    ...
    with st.sidebar:
        st.header("⚙️ 설정")
        owner = st.text_input("GitHub 사용자명", value="your-username")
        repo  = st.text_input("레포지터리명",   value="short-tracker-263750")
